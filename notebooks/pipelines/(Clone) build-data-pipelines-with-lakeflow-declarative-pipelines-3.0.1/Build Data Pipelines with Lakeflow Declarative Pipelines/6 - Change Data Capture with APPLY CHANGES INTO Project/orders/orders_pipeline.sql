...
-- 1. Review the code below. Notice that each streaming table or materialized view now includes a COMMENT and TBLPROPERTIES section to document each object.  
  ...
  -- NOTE: In the end, it's the same orders pipeline we've reviewed in the previous demostrations.
...
CREATE OR REFRESH STREAMING TABLE 1_bronze_db.orders_bronze_demo6
...
CREATE OR REFRESH STREAMING TABLE 2_silver_db.orders_silver_demo6
  (
...
SELECT 
  ...
  customer_id,
  notifications
FROM STREAM 1_bronze_db.orders_bronze_demo6; 


-- Create the materialized view aggregation from the orders_silver table with the summarization
CREATE OR REFRESH MATERIALIZED VIEW 3_gold_db.orders_by_date_gold_demo6 
...
FROM 2_silver_db.orders_silver_demo6  
