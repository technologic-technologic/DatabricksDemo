...
@dlt.table(name = "1_bronze_db.status_bronze_demo5",
...
@dlt.table(name = "2_silver_db.status_silver_demo5",
...
# Expectations
@dlt.expect_or_drop("valid_timestamp", "order_status_timestamp > '2021-12-25'")
@dlt.expect("valid_order_status", "order_status IN ('on the way','canceled','return canceled','delivered','return processed','placed','preparing')")

def status_silver_demo5():
    return (
        dlt.read_stream("1_bronze_db.status_bronze_demo5")
            .select(
                "order_id",
                "order_status",
...
@dlt.table(
    name = "3_gold_db.full_order_info_gold_demo5",
...
def full_order_info_gold_demo5():
    return (
        dlt
        .read("2_silver_db.status_silver_demo5").alias("status")
        .join(dlt.read("2_silver_db.orders_silver_demo5").alias("orders"), on = "order_id", how = "inner")
        .select(
            "orders.order_id",
...
## CANCELLED ORDERS
@dlt.table(
    name="3_gold_db.cancelled_orders_gold_demo5",
...
def cancelled_orders_gold_demo5():
    full_orders = dlt.read("3_gold_db.full_order_info_gold_demo5")
    return (
        full_orders
...
## DELIVERED ORDERS
@dlt.table(
    name="3_gold_db.delivered_orders_gold_demo5",
