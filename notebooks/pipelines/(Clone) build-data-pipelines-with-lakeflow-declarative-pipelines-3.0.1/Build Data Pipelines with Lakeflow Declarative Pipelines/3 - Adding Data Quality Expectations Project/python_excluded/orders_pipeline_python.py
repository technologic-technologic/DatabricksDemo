...
@dlt.table(name="1_bronze_db.orders_bronze_demo3")
def orders_bronze_demo3():
    return (
        spark.readStream
            .format("cloudFiles")
...
@dlt.table(name="2_silver_db.orders_silver_demo3")

# Expectations
@dlt.expect("valid_notifications", "notifications IN ('Y','x')")
@dlt.expect_or_drop("valid_date", F.col("order_timestamp") > "2021-12-26")
@dlt.expect_or_fail("valid_id", F.col("customer_id").isNotNull())

def orders_silver_demo3():
    return (
        dlt.read_stream("1_bronze_db.orders_bronze_demo3")
            .select(
                "order_id",
...
@dlt.table(name="3_gold_db.gold_orders_by_date_demo3")
def orders_by_date_gold_demo3():
    return (
        dlt.read("2_silver_db.orders_silver_demo3")
